...
class QuestEval:
    ...
    def _base_score(
        ...
        # 🛠️ Cambio realizado: función robusta para controlar puntuaciones tipo lista anidada
        #def list_borned(a_list):
            #flat = []
            #for x in a_list:
                #if isinstance(x, list):  # si viene como lista anidada
                    #x = x[0]             # usamos el primer elemento (puedes ajustar si prefieres promedio)
                #flat.append(max(min(1, x), 0))
            #return flat
