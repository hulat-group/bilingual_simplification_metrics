...
class Inferencer():
    def __init__(self, ckpt_path, model='bert-base-uncased', batch_size=32, device='cuda', verbose=True) -> None:
        ...
        if ckpt_path is not None:
            ...
            self.model = BERTAlignModel.load_from_checkpoint(checkpoint_path=ckpt_path, strict=False, model=model).to(self.device) #PARA ESPAÑOL
        else:
        ...
        self.spacy = spacy.load('en_core_web_sm') #PARA INGLES
        #self.spacy = spacy.load('es_core_news_sm') #PARA ESPAÑOL
